...
def _get_centrality(density: int, is_brand_title: bool, has_brand_in_text: bool) -> int:
    ...
    Density calibration note: process_article_intel adds brand_bonus=+10 whenever
    a CORE_BRAND appears. This inflates raw density, so thresholds for text-only
    brand mentions are set accordingly:
      density >= 14  →  brand + 3+ distinct ecosystem keywords (substantive coverage)
      density 12–13  →  brand + 1–2 ecosystem keywords (moderate coverage)
      density 11     →  brand + just its own KEYWORDS entry (single peripheral mention)
      density 10     →  brand present via RSS fallback with zero other keyword hits
    """
    if is_brand_title and density >= 10:
        ...
        return 6
    elif has_brand_in_text and density >= 14:
        return 6   # substantive: brand + 3+ ecosystem keyword matches
    elif has_brand_in_text and density >= 12:
        return 3   # moderate: brand + 1–2 ecosystem keywords
    elif has_brand_in_text:
        return 1   # peripheral: single mention, no surrounding ecosystem context
    elif density >= 3:
        return 3
    else:
        return 2
...
def compute_scores(item: dict) -> dict:
    ...
    # Peripheral mention gate: brand appears in text but NOT in the title and
    # density is at its minimum (brand_bonus=10 + 1 keyword = 11, meaning no
    # additional ecosystem keywords were matched). The article mentions OpenClaw
    # in passing but is not substantively about it. Source credibility (D4)
    # should not be able to carry such an article above genuinely on-topic content.
    _peripheral = has_brand_text and (not is_brand_title) and (density < 12)
    ...
    # Peripheral mention cap: article mentions the brand but OpenClaw is not the
    # subject. D4 source credibility is preserved as-is, but the total_score is
    # capped at 30 so that a single peripheral mention from a reputable publisher
    # cannot outrank articles where OpenClaw is substantively covered.
    if _peripheral:
    ...
    if _peripheral:
        stage_tags.append('peripheral-mention')
